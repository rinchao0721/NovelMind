...
async def run_analysis(task_id: str, config: AnalysisConfig):
    ...
    try:
        ...
        async with get_db() as db:
            ...
            await db.execute(
                "UPDATE novels SET analysis_status = 'failed' WHERE id = ?",
                (config.novel_id,)
            )
            await db.commit()
    finally:
...
async def start_analysis(config: AnalysisConfig, background_tasks: BackgroundTasks):
    ...
    async with get_db() as db:
        ...
        # Update novel status
        await db.execute(
            "UPDATE novels SET analysis_status = 'analyzing' WHERE id = ?",
            (config.novel_id,)
        )
        await db.commit()
...
async def get_analysis_result(task_id: str):
    ...
    async with get_db() as db:
        # Get task info
        cursor = await db.execute(
            "SELECT novel_id, status FROM analysis_tasks WHERE id = ?",
            (task_id,)
        )
        row = await cursor.fetchone()
...
async def cancel_analysis(task_id: str):
    ...
    async with get_db() as db:
        await db.execute(
            "UPDATE analysis_tasks SET status = 'cancelled' WHERE id = ?",
            (task_id,)
        )
        await db.commit()
