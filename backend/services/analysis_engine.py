...
class AnalysisEngine:
    ...
    async def analyze(
        ...
        # Update progress
        total_steps = len([f for f in features if f in ['characters', 'relationships', 'plot', 'summary']])
        current_step = 0
            ...
            async with get_db() as db:
                for char in all_characters.values():
                    await db.execute(
                        ...
                        """,
                        (
                            char['id'], char['novel_id'], char['name'],
                            ','.join(char['aliases']), char['description'],
                            char['personality'], char['first_appearance'],
